	...
	return (
		<div className="user__column">
			<li className="user__item item row d-md-flex flex-direction-md-column align-items-md-center" key={user.id} onClick={() => getSelected(user)}>
				...
				<div className="item__name col-12 col-md-2 my-1 text-center">{user.fname} {user.name.charAt(0)}. {user.mname.charAt(0)}.</div>
				<div className="item__balance col-12 col-md-2 my-0 text-md-start text-center">Баланс: {Math.round(user.balance * Math.pow(10, 2)) / Math.pow(10, 2)}</div>
				<div className="item__last-updated col-12 col-md-5 my-1 text-md-end text-center">Последнее изменение: {moment(user.lastUpdatedAt).fromNow()}</div>
				<div className="item__dropdown-2 col-12 col-md-2 my-1 text-md-end text-center" onClick = {e => e.stopPropagation()}>
